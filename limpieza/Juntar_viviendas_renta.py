...
def process_and_merge_data(viviendas_path, renta_path, output_path=None):
    ...
    renta_duplicates = renta[renta.duplicated(subset=['localidad_mapeada', 'Periodo'], keep=False)]
    print(f"Duplicados en renta:\n{renta_duplicates}")

    # Eliminar duplicados
    renta = renta.drop_duplicates(subset=['localidad_mapeada', 'Periodo'])
